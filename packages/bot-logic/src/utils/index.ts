...
// TODO: Definition
export const getTotalQuantityFromReservations = (
  reservations: Reservation[]
) => {
  return reservations
    .map(r => r.reservedQuantity)
...
export const getUniqueReservations = (reservations: Reservation[]) => {
  return reservations.filter(
    (value, index, self) =>
      index === self.findIndex(t => t.variantId === value.variantId)
  );
};
...
export function tryGetNewReservationQuantity(
  e: AxiosError,
  currentQnt: number
) {
