...
const OWNER_SOCKET_ID = 'asdfasdf';
    ...
    describe('getTicketVariants', () => {
      it('Should get array of variants even if no variants are available at first', async () => {
        ...
        const expectedVariants =
          eventVariantsFirstMissing.pageDataVariants.model.variants;
      ...
      it('Should wait for sales to start if sales have not started yet', async () => {
        ...
        const expectedVariants =
          eventSalesNotStarted.pageDataVariants.model.variants;
        const variants = await bot['getTicketVariants'](eventUrl);
          ...
          rest.post(
            ...
            async (req, res, ctx) => {
              ...
              if (quantity === 2) {
                return res(
                  ctx.status(200),
                  ctx.json(eventSalesNotStarted.responseReserved)
                );
              }
              if (quantity > 2) {
                return res(
      ...
      server.use(
        ...
        rest.post(
          ...
          async (req, res, ctx) => {
            ...
            const resData = await createSuccesReservationByInventoryId(
              inventoryId,
              quantity
            );
